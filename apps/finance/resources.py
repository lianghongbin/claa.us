...
def _normalize_type(raw: str | None) -> str:
    ...
    raise ValueError(f"无法识别的收支类型: {raw}")
...
class TransactionResource(resources.ModelResource):
    ...
    category_name = fields.Field(column_name="科目名称", attribute=None, readonly=True)
    project_code = fields.Field(column_name="项目编码", attribute=None, readonly=True)
    counterparty_code = fields.Field(column_name="单位编码", attribute=None, readonly=True)
    tag_names = fields.Field(column_name="标签", attribute=None, readonly=True)
    ...
    def before_import_row(self, row, row_number=None, **kwargs) -> None:
        ...
        raw_type = data.get("transaction_type") or data.get("收支类型")
        if fixed is not None:
            if raw_type in (None, ""):
                ...
                if resolved != fixed:
                    label = "收入" if fixed == TransactionType.INCOME else "支出"
                    raise ValueError(
                        f"第 {row_number} 行: 收支类型与当前页面（{label}明细）不一致。"
                    )
                row["_resolved_type"] = resolved
        else:
            row["_resolved_type"] = _normalize_type(raw_type)

        name = (data.get("category_name") or data.get("科目名称") or "").strip()
        if not name:
        ...
        for part in str(raw_tags).split(","):
            ...
            if not name:
                continue
            try:
                tag_ids.append(Tag.objects.get(name=name).pk)
            except Tag.DoesNotExist as exc:
                raise ValueError(
                    f"第 {row_number} 行: 未找到标签「{name}」，请先在后台维护标签。"
                ) from exc
        row["_resolved_tag_ids"] = tag_ids

    def get_import_fields(self):
        """虚拟列在 import_instance 中统一处理。"""
        return []
    ...
    def import_instance(self, instance, row, **kwargs):
        ...
        raw_date = data.get("date") or data.get("日期")
        ...
        if hasattr(raw_date, "year"):
            ...
            if not parsed:
                raise ValueError(f"日期格式无效: {raw_date}")
            instance.date = parsed

        acc = (data.get("account_name") or data.get("账户名称") or "").strip()
        if not acc:
            raise ValueError("账户名称不能为空")
        instance.account_name = acc

        irr = data.get("is_reconciled") or data.get("已对账")
